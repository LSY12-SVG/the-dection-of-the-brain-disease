...
DEFAULT_DATA = BASE_DIR / '肿瘤模型数据集/images/test'
...
def run_pipeline(data_dir, det_weights, seg_weights, out_csv, save_vis=False):
    ...
    if Path(det_weights).exists():
        ...
        for i, img_path in enumerate(image_files):
            ...
            preds = model_det.predict(source=img_path, conf=0.25, save=save_vis, verbose=False, project='runs/detect', name='predict_det', exist_ok=True)
            res = preds[0]
    ...
    if Path(seg_weights).exists():
        ...
        for i, img_path in enumerate(image_files):
            preds = model_seg.predict(source=img_path, conf=0.35, save=save_vis, verbose=False, project='runs/segment', name='predict_seg', exist_ok=True)
            res = preds[0]
            fname = Path(res.path).name
            has_tumor = 0
