...
def run_pipeline(data_dir, det_weights, seg_weights, out_csv, save_vis=False):
    ...
    if Path(det_weights).exists():
        ...
        viz_dir = Path(out_csv).resolve().parent / 'viz_detect'
        if save_vis:
            viz_dir.mkdir(parents=True, exist_ok=True)
        
        for i, img_path in enumerate(image_files):
            # Run inference on single image
            preds = model_det.predict(source=img_path, conf=0.25, save=False, verbose=False)
            res = preds[0]
            ...
            if save_vis:
                try:
                    ...
                    save_path = viz_dir / Path(img_path).name
                    Image.fromarray(im_rgb).save(str(save_path))
    ...
    if Path(seg_weights).exists():
        ...
        # Define project path relative to output CSV
        viz_dir = Path(out_csv).resolve().parent / 'viz_segment'
        if save_vis:
            viz_dir.mkdir(parents=True, exist_ok=True)
        
        for i, img_path in enumerate(image_files):
            preds = model_seg.predict(source=img_path, conf=0.35, save=False, verbose=False)
            res = preds[0]
...
def main():
    ...
    parser.add_argument('--out', type=str, default='./输出结果/cla_pre.csv', help='Output CSV path')
    parser.add_argument('--save', action='store_true', default=True, help='Save visualization images')
